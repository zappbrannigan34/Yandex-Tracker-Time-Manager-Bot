...
- ✅ 2 режима запуска: `sync` и `sync --dry-run`
...
# Полный запуск (бекфилл месяца + сегодняшний день)
./time-tracker-bot sync

# Зеркалирование вывода в файл (удобно при долгих прогонов)
./time-tracker-bot sync --dry-run --tee-output logs/manual-sync.log
```

### 📊 Month-to-Date Tracking & Backfill

`sync` всегда работает в два этапа:

1. Анализирует месяц по timeline задач, выводит сводку и в реальном режиме дозаписывает все пропущенные рабочие дни (кроме будущих дат).
2. Распределяет сегодняшний день по ежедневным/еженедельным правилам и задачам, которые реально были в работе сегодня.

В режиме dry-run показана вся статистика, но без создания worklog'ов:
...
> ⚠️ Секция сохранила имя `daemon` ради обратной совместимости, но теперь управляет только путём и уровнем логирования единой команды `sync`.

### 5. IAM Token

```yaml
iam:
  # Как часто проверять необходимость обновления токена
  # Токен обновляется ТОЛЬКО если истекает в течение 1 часа
  # IAM токены живут до 12 часов, поэтому рекомендуется проверять каждый час
  refresh_interval: "1h"

  # CLI команда для получения IAM токена
  cli_command: "yc iam create-token"  # или полный путь на Windows

  # Опциональная команда для автоматического yc init (SSO/federation)
  # init_command: "yc init --federation-id=YOUR_FEDERATION_ID"

  # Или просто укажите federation_id, чтобы добавить флаг к стандартному init
  # federation_id: "YOUR_FEDERATION_ID"
```
...
# Dry-run end-to-end проверка
./time-tracker-bot sync --dry-run
```

**Результаты (2025-11-11):**
- ✅ API authentication (SSO/Cloud Organizations)
- ✅ dry-run sync: 6 entries, 7h 2m distributed (без создания worklogs)
